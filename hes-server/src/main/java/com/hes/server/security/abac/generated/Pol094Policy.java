...
@Component
public class Pol094Policy implements AbacPolicy {
    @Override public String id() { return "POL-094"; }
    @Override public String description() { return "Allow MANAGE_SITE only when subject role includes OPERATOR"; }
    @Override
    public boolean permits(AbacRequest request) {
        if (request == null || request.action() == null) { return false; }
        if (!"MANAGE_SITE".equals(request.action())) { return true; }
