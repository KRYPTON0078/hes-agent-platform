...
@Component
public class Ano144Detector implements AnomalyDetector {
    @Override public String id() { return "ANO-144"; }
    @Override public String description() { return "Detect SOC_DROP when numeric value exceeds 34"; }
    @Override
    public Optional<AnomalyFinding> detect(AnomalySignal signal) {
        if (signal == null || signal.signalType() == null) { return Optional.empty(); }
        if ("SOC_DROP".equals(signal.signalType()) && signal.numericValue() > 34) {
            return Optional.of(new AnomalyFinding(id(), signal.deviceId(),
                "SOC_DROP exceeded threshold 34", signal.numericValue() / 34));
        }
