...
@Component
public class Ano080Detector implements AnomalyDetector {
    @Override public String id() { return "ANO-080"; }
    @Override public String description() { return "Detect SOC_DROP when numeric value exceeds 10"; }
    @Override
    public Optional<AnomalyFinding> detect(AnomalySignal signal) {
        if (signal == null || signal.signalType() == null) { return Optional.empty(); }
        if ("SOC_DROP".equals(signal.signalType()) && signal.numericValue() > 10) {
            return Optional.of(new AnomalyFinding(id(), signal.deviceId(),
                "SOC_DROP exceeded threshold 10", signal.numericValue() / 10));
        }
