...
@Component
public class Ano010Detector implements AnomalyDetector {
    @Override public String id() { return "ANO-010"; }
    @Override public String description() { return "Detect OFFLINE_BURST when numeric value exceeds 20"; }
    @Override
    public Optional<AnomalyFinding> detect(AnomalySignal signal) {
        if (signal == null || signal.signalType() == null) { return Optional.empty(); }
        if ("OFFLINE_BURST".equals(signal.signalType()) && signal.numericValue() > 20) {
            return Optional.of(new AnomalyFinding(id(), signal.deviceId(),
                "OFFLINE_BURST exceeded threshold 20", signal.numericValue() / 20));
        }
