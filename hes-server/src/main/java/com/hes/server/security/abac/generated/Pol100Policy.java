...
@Component
public class Pol100Policy implements AbacPolicy {
    @Override public String id() { return "POL-100"; }
    @Override public String description() { return "Allow READ_AUDIT only when subject role includes OPERATOR"; }
    @Override
    public boolean permits(AbacRequest request) {
        if (request == null || request.action() == null) { return false; }
        if (!"READ_AUDIT".equals(request.action())) { return true; }
