...
@Component
public class Ano028Detector implements AnomalyDetector {
    @Override public String id() { return "ANO-028"; }
    @Override public String description() { return "Detect AUTH_FAIL_RATE when numeric value exceeds 38"; }
    @Override
    public Optional<AnomalyFinding> detect(AnomalySignal signal) {
        if (signal == null || signal.signalType() == null) { return Optional.empty(); }
        if ("AUTH_FAIL_RATE".equals(signal.signalType()) && signal.numericValue() > 38) {
            return Optional.of(new AnomalyFinding(id(), signal.deviceId(),
                "AUTH_FAIL_RATE exceeded threshold 38", signal.numericValue() / 38));
        }
