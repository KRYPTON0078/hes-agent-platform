...
@Component
public class Ano037Detector implements AnomalyDetector {
    @Override public String id() { return "ANO-037"; }
    @Override public String description() { return "Detect TELEMETRY_GAP when numeric value exceeds 47"; }
    @Override
    public Optional<AnomalyFinding> detect(AnomalySignal signal) {
        if (signal == null || signal.signalType() == null) { return Optional.empty(); }
        if ("TELEMETRY_GAP".equals(signal.signalType()) && signal.numericValue() > 47) {
            return Optional.of(new AnomalyFinding(id(), signal.deviceId(),
                "TELEMETRY_GAP exceeded threshold 47", signal.numericValue() / 47));
        }
