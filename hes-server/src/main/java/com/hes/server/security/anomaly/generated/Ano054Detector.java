...
@Component
public class Ano054Detector implements AnomalyDetector {
    @Override public String id() { return "ANO-054"; }
    @Override public String description() { return "Detect KEY_ROTATE_BURST when numeric value exceeds 24"; }
    @Override
    public Optional<AnomalyFinding> detect(AnomalySignal signal) {
        if (signal == null || signal.signalType() == null) { return Optional.empty(); }
        if ("KEY_ROTATE_BURST".equals(signal.signalType()) && signal.numericValue() > 24) {
            return Optional.of(new AnomalyFinding(id(), signal.deviceId(),
                "KEY_ROTATE_BURST exceeded threshold 24", signal.numericValue() / 24));
        }
