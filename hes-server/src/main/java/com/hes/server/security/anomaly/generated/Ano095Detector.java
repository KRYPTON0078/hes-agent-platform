...
@Component
public class Ano095Detector implements AnomalyDetector {
    @Override public String id() { return "ANO-095"; }
    @Override public String description() { return "Detect EXPORT_SPIKE when numeric value exceeds 25"; }
    @Override
    public Optional<AnomalyFinding> detect(AnomalySignal signal) {
        if (signal == null || signal.signalType() == null) { return Optional.empty(); }
        if ("EXPORT_SPIKE".equals(signal.signalType()) && signal.numericValue() > 25) {
            return Optional.of(new AnomalyFinding(id(), signal.deviceId(),
                "EXPORT_SPIKE exceeded threshold 25", signal.numericValue() / 25));
        }
