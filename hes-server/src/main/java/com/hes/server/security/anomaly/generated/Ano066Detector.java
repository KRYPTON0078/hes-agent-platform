...
@Component
public class Ano066Detector implements AnomalyDetector {
    @Override public String id() { return "ANO-066"; }
    @Override public String description() { return "Detect OFFLINE_BURST when numeric value exceeds 36"; }
    @Override
    public Optional<AnomalyFinding> detect(AnomalySignal signal) {
        if (signal == null || signal.signalType() == null) { return Optional.empty(); }
        if ("OFFLINE_BURST".equals(signal.signalType()) && signal.numericValue() > 36) {
            return Optional.of(new AnomalyFinding(id(), signal.deviceId(),
                "OFFLINE_BURST exceeded threshold 36", signal.numericValue() / 36));
        }
