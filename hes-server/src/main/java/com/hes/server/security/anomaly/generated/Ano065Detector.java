...
@Component
public class Ano065Detector implements AnomalyDetector {
    @Override public String id() { return "ANO-065"; }
    @Override public String description() { return "Detect CMD_FLOOD when numeric value exceeds 35"; }
    @Override
    public Optional<AnomalyFinding> detect(AnomalySignal signal) {
        if (signal == null || signal.signalType() == null) { return Optional.empty(); }
        if ("CMD_FLOOD".equals(signal.signalType()) && signal.numericValue() > 35) {
            return Optional.of(new AnomalyFinding(id(), signal.deviceId(),
                "CMD_FLOOD exceeded threshold 35", signal.numericValue() / 35));
        }
